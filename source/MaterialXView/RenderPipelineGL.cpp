...
{
    renderFrame(nullptr, shadowMapSize, dirLightNodeCat, 0, 0);
}

uint64_t GLRenderPipeline::renderFrame(void*, int shadowMapSize, const char* dirLightNodeCat, int warmupFrames, int benchmarkFrames)
{
    auto& genContext    = _viewer->_genContext;
    auto& lightHandler  = _viewer->_lightHandler;
    auto& imageHandler  = _viewer->_imageHandler;
